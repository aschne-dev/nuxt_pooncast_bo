<template>
  <div>
    <div class="bg-tertiary grid grid-cols-2 rounded-t-lg p-2 font-nunito gap-5">
      <div class="border-black rounded-lg p-2 border">
        <p>{{ participation.firstName }}</p>
        <p>{{ participation.age }} ans</p>
        <p>{{ participation.city }}</p>
        <p>{{ participation.email }}</p>
      </div>

      <div class="text-start border border-black p-2 rounded-lg">
        <p>{{ participation.description }}</p>
      </div>
    </div>

    <div class="bg-tertiary grid grid-cols-1 rounded-b-lg p-2 relative">
      <div class="flex flex-col items-center">
        <div class="">
          <audio controls :src="participation.fileUrl"></audio> 
        </div>

        <div class="flex items-center justify-center">
          <button class="p-2 rounded-full opacity-80" @click="participationsStore.toggleProcessed(participation.docid)">
            <svg  class="size-10" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
            :class="{ 'fill-secondary': participation.status == 'processed' }">
              <path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
            </svg>

          </button>        

          <button class="p-2 rounded-full opacity-80" @click="participationsStore.toggleFav(participation.docid)">
            <svg class="size-10" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
            :class="{ 'fill-secondary': participation.fav == 1 }">
              <path stroke-linecap="round" stroke-linejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12Z" />
            </svg>
          </button>
        </div>
      </div>

      <div class="absolute bottom-1 right-1">
        <button class="bg-red-500 px-3 py-3 rounded-full opacity-80 hover:bg-gray-600" @click="handleDelete(participation.docid)">
          <svg data-slot="icon" fill="none" stroke-width="1.5" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" class="size-5">
          <path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0"></path>
          </svg>
        </button>
      </div>

    </div>

  </div>
</template>

<script setup>
import { useParticipationsStore } from '@/stores/Participations/participation';
const participationsStore = useParticipationsStore();

const props = defineProps({
  participation: Object,
});

const handleDelete = (docid) => {
  if (confirm('Êtes-vous sûr de vouloir supprimer cette participation ? Cette action est irréversible.')) {
    participationsStore.deleteParticipation(docid);
  }
};
</script>
