<template>
    <div class="">
        <div class="flex items-center justify-between">
            <div class="bg-primary py-5 px-5 rounded-xl font-syne font-medium text-3xl cursor-pointer w-full"
                @click="toggleShowAnswer">
                <div class="flex justify-between items-center">
                    {{ faq.question }}
                    <svg :class="{ rotated: isOpen }" class="transition-transform duration-200 ease-in" xmlns="http://www.w3.org/2000/svg" width="24" height="42" viewBox="0 0 24 42" fill="none">
            ...
            <div class="flex items-center justify-center gap-2 ml-2">
                <button class="bg-gray-400 px-3 py-3 rounded-full opacity-80 hover:bg-gray-600" @click="toggleEditFaq">
                    <svg v-if="!showEditFaq" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5">
        ...
        <!-- EDIT -->
        <div v-if="showEditFaq" class="border-t-2 mt-5 pb-5 border-b-2">
            <EditFaq :faq="props.faq" @faqUpdated="showEditFaq = false"/>
        </div>
    </div>
</template>
...
const toggleShowAnswer = () => {
  emit('toggleFaq', props.faq.id);
};


const handleDelete = async () => {
  const confirmed = confirm(`Supprimer la FAQ de ${props.faq.question} ?`);
  if (confirmed) {
    await faqStore.deleteFaq(props.faq.id);
  }
};

const toggleEditFaq = () => {
    showEditFaq.value = !showEditFaq.value
}
